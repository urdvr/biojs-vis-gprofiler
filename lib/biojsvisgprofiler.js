 ...
 * @property {boolean} logoHttpPost - Set to true to use a HTTP POST request
...
BioJSVisGProfiler.prototype._renderCloud = function(clels) {
  ...
  clels =
    _this._capClels(clels, clelScores);
  clelScores =
    _this._normalizeClelScores(clels, clelScores);
...
BioJSVisGProfiler.prototype._renderDraw = function(words, scaling) {
  ...
  if (_this.showLogo) {
    ...
    logo = d3c.select('#' + _this.LOGO_ID);

    if (!_this.logoHttpPost) {
      logo
        .attr('xlink:href', gprofiler.getQueryURL() || gprofiler.getRootURL())
        .attr('target', '_blank');
    }
    else {
      logo.on('click', function() { _this._submitForm(); });
    }
  }
...
BioJSVisGProfiler.prototype._submitForm = function() {

  // Submit a form to g:Profiler (upon clicking the logo, if HTTP POST
  // requests enabled)

  var _this = this;
  var $ = _this.$;
  var _ = _this._;
  var form = $('#' + _this.POSTFORM_ID);
  var html = '';
  var gp = _this.getGProfiler();

  // Append form DOM node if not present

  if (!form.length) {
    html =
      '<form id="' + _this.POSTFORM_ID + '" method="post" ' +
      'action="' + gp.getRootURL() + '"></form>';

    $('body').append(html);
    form = $('#' + _this.POSTFORM_ID);
  ...
  $.each(gp.getQueryParams(), function(k, v) {
    var input =
      '<input type="text" name="' + _.escape(k) + '" ' +
      'value="' + _.escape(v) + '" />';
    form.append(input);
  });
...
BioJSVisGProfiler.prototype._capClels = function(clels, clelScores) {
...
BioJSVisGProfiler.prototype._normalizeClelScores = function(clels, clelScores) {

  // Scale scores, total == 1; but leave only those present in clels.
  ...
  $.each(clels, function(i, v) {
    ...
    totalScore += score;
    newClelScores[str] = score;
  });
  $.each(newClelScores, function(k, v) {
    newClelScores[k] = newClelScores[k] / totalScore;
  });

  return newClelScores;
};
