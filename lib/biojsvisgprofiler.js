...
BioJSVisGProfiler.prototype._renderCloud = function(clels) {
  ...
  cloud().size([w, h])
    ...
    .on('end', function(words) {
      _this._renderDraw.apply(_this, [words]);
    })
    .start();
};
...
function(clels, clelScores, scaling) {
  ...
  r = $.map(clels, function(v, i) {
    ...
    var s = sizer(score, scaling, str, termdata);
    var minS = 12;
    var maxS = 72 * scaling;
...
BioJSVisGProfiler.prototype._renderDraw = function(words) {
...
BioJSVisGProfiler.prototype._defaultCallbacks = function() {
  ...
  var fill = _this.d3.scale.category20();
  var filli = 0;
  ...
  cb.sizerTerm = function(score, scaling, str, tdata) {
      return score * 1000 * scaling;
  };

  // Colorer callback

  cb.colorerWord = cb.colorerTerm = function(score, scaling, str, tdata) {
    return fill(++filli);
  };
